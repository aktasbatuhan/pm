...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _kai_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _kai_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _kai_now().strftime("%Y-%m-%d_%H-%M-%S")
