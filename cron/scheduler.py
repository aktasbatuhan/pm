...
# Resolve Kai home directory (respects KAI_HOME / HERMES_HOME override)
_kai_home = kai_home()

# File-based lock prevents concurrent ticks from gateway + daemon + systemd timer
_LOCK_DIR = _kai_home / "cron"
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if origin:
        os.environ["KAI_SESSION_PLATFORM"] = origin["platform"]
        os.environ["HERMES_SESSION_PLATFORM"] = origin["platform"]
        os.environ["KAI_SESSION_CHAT_ID"] = str(origin["chat_id"])
        os.environ["HERMES_SESSION_CHAT_ID"] = str(origin["chat_id"])
        if origin.get("chat_name"):
            os.environ["KAI_SESSION_CHAT_NAME"] = origin["chat_name"]
    ...
    try:
        ...
        try:
            load_dotenv(str(_kai_home / ".env"), override=False, encoding="utf-8")
        ...
        try:
            import yaml
            _cfg_path = str(_kai_home / "config.yaml")
        ...
        prefill_file = get_env("KAI_PREFILL_MESSAGES_FILE") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            import json as _json
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _kai_home / pfpath
            if pfpath.exists():
                try:
        ...
        # Load shared workspace context so cron jobs have cross-thread awareness
        _ws_prompt = None
        try:
            ...
            _ws_prompt = build_workspace_status_prompt(_ws_status) if _ws_status else None
        except Exception as _wce:
            logger.debug("Workspace context load failed (non-fatal): %s", _wce)

        _cron_session_id = f"cron_{job_id}_{_kai_now().strftime('%Y%m%d_%H%M%S')}"
        agent = AIAgent(
            model=model,
        ...
        result = agent.run_conversation(prompt, system_message=_ws_prompt)
...
**Job ID:** {job_id}
**Run Time:** {_kai_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        for key in ("KAI_SESSION_PLATFORM", "KAI_SESSION_CHAT_ID", "KAI_SESSION_CHAT_NAME",
...
def tick(verbose: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _kai_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _kai_now().strftime('%H:%M:%S'), len(due_jobs))
