...
async def cache_image_from_url(url: str, ext: str = ".jpg") -> str:
    ...
    async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
        response = await client.get(
            url,
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; DashPM/1.0)",
                "Accept": "image/*,*/*;q=0.8",
            },
        )
...
class BasePlatformAdapter(ABC):
    ...
    def _get_human_delay() -> float:
        ...
        mode = (os.getenv("KAI_HUMAN_DELAY_MODE") or os.getenv("HERMES_HUMAN_DELAY_MODE", "off")).lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("KAI_HUMAN_DELAY_MIN_MS") or os.getenv("HERMES_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("KAI_HUMAN_DELAY_MAX_MS") or os.getenv("HERMES_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
