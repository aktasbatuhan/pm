...
# Load project .env FIRST so KAI_HOME/HERMES_HOME are set before kai_home() resolves
...
_hermes_home = kai_home()
# Also load the home-dir .env (won't overwrite keys already set by project .env)
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["KAI_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
                os.environ["HERMES_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
        # Timezone: bridge config.yaml → KAI_TIMEZONE env var.
        # KAI_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "KAI_TIMEZONE" not in os.environ and "HERMES_TIMEZONE" not in os.environ:
            os.environ["KAI_TIMEZONE"] = _tz_cfg.strip()
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=get_env("KAI_INFERENCE_PROVIDER"),
        )
...
def _resolve_gateway_model() -> str:
    ...
    """
    model = get_env("KAI_MODEL") or os.getenv("LLM_MODEL") or "anthropic/claude-opus-4.6"
    try:
        import yaml as _y
...
class GatewayRunner:
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks KAI_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.hermes/config.yaml.
        Relative paths are resolved from ~/.hermes/.
        """
        import json as _json
        file_path = get_env("KAI_PREFILL_MESSAGES_FILE")
        if not file_path:
            try:
                import yaml as _y
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks KAI_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.hermes/config.yaml.
        """
        prompt = get_env("KAI_EPHEMERAL_SYSTEM_PROMPT")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        Checks KAI_REASONING_EFFORT env var first, then agent.reasoning_effort
        in config.yaml. Valid: "xhigh", "high", "medium", "low", "minimal", "none".
        Returns None to use default (medium).
        """
        effort = get_env("KAI_REASONING_EFFORT")
        if not effort:
            try:
                import yaml as _y
                cfg_path = _hermes_home / "config.yaml"
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = get_env("KAI_BACKGROUND_NOTIFICATIONS")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _hermes_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Dash PM Gateway...")
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        # First-message onboarding — trigger self-onboard on fresh workspace
        if not history and not self.session_store.has_any_sessions():
            # Mark onboarding as started in workspace context
            try:
                from workspace_context_bridge import fetch_workspace_status, update_workspace_status
                _onb_status = fetch_workspace_status()
                if _onb_status and _onb_status.get("onboardingStatus") == "not_started":
                    update_workspace_status(
                        os.environ.get("KAI_WORKSPACE_ID", os.environ.get("HERMES_WORKSPACE_ID", "default")),
            ...
            context_prompt += (
                ...
                "You are a new engineer on your first day. Your workspace context tells you "
                "what to do — follow it. Start by loading the self-onboard skill and exploring.]"
            )
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Dash delivers cron job results "
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        lines = [
            ...
            "`/update` — Update Dash PM to the latest version",
    ...
    async def _handle_model_command(self, event: MessageEvent) -> str:
        ...
        current = get_env("KAI_MODEL") or "anthropic/claude-opus-4.6"
        ...
        os.environ["KAI_MODEL"] = new_model
        os.environ["HERMES_MODEL"] = new_model
        if provider_changed:
            os.environ["KAI_INFERENCE_PROVIDER"] = target_provider
    ...
    async def _run_background_task(
        ...
        try:
            ...
            default_toolset_map = {
                Platform.LOCAL: "kai-cli",
                Platform.TELEGRAM: "kai-telegram",
                Platform.DISCORD: "kai-discord",
                Platform.WHATSAPP: "kai-whatsapp",
                Platform.SLACK: "kai-slack",
                Platform.SIGNAL: "kai-signal",
                Platform.HOMEASSISTANT: "kai-homeassistant",
                Platform.EMAIL: "kai-email",
            }
            ...
            if config_toolsets and isinstance(config_toolsets, list):
                ...
                default_toolset = default_toolset_map.get(source.platform, "kai-telegram")
                enabled_toolsets = [default_toolset]

            platform_key = "cli" if source.platform == Platform.LOCAL else source.platform.value

            pr = self._provider_routing
            max_iterations = int(get_env("KAI_MAX_ITERATIONS", "90"))
    ...
    async def _handle_jobs_command(self, event: MessageEvent) -> str:
        """Handle /jobs command — show GitHub PRs, reviews, and assigned issues."""
        try:
            from tools.github_jobs import run_jobs_command
        except ImportError as exc:
            return f"❌ /jobs requires httpx: {exc}"

        args = event.get_command_args().strip()
        try:
            return run_jobs_command(args)
        except Exception as exc:
            logger.exception("Error in /jobs command")
            return f"❌ Error fetching GitHub jobs: {exc}"
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        return "⚕ Starting Dash PM update… I'll notify you when it's done."
        ...
        try:
            ...
            if adapter and chat_id:
                ...
                if output:
                    ...
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    msg = f"✅ Dash PM update finished — gateway restarted.\n\n```\n{output}\n```"
                else:
                    msg = "✅ Dash PM update finished — gateway restarted successfully."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["KAI_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["HERMES_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["KAI_SESSION_CHAT_ID"] = context.source.chat_id
    ...
    async def _run_agent(
        ...
        progress_mode = (
            ...
            or get_env("KAI_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["KAI_SESSION_KEY"] = session_key or ""
            ...
            # Load shared workspace context — gives the agent cross-thread awareness
            _ws_prompt = ""
            try:
                from workspace_context_bridge import fetch_workspace_status, build_workspace_status_prompt
                _ws_status = fetch_workspace_status()
                _ws_prompt = build_workspace_status_prompt(_ws_status) if _ws_status else ""
            ...
            # Create state reporter for backend telemetry (activities, patterns, sessions)
            ...
            try:
                ...
                pass  # StateReporter is optional — agent works without it
            ...
            _agent_start_time = _time_mod.time()
            result = agent.run_conversation(
                message,
                system_message=_ws_prompt or None,
                conversation_history=agent_history,
                task_id=session_id,
            )
            result_holder[0] = result

            # Report session to backend after agent finishes
            if _state_reporter:
                try:
                    _tool_calls = sum(
                    ...
                    _state_reporter.report_session(
                        session_type="message",
                        description=message[:200] if message else "chat",
                        tool_call_count=_tool_calls,
                        duration_ms=int((_time_mod.time() - _agent_start_time) * 1000),
                    )
                    _state_reporter.stop()
                except Exception:
                    pass
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False) -> bool:
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            hermes_home = get_env("KAI_HOME", "~/.kai-agent")
            logger.error(
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Dash PM Gateway - Multi-platform messaging")
