...
class MCPServerTask:
    ...
    __slots__ = (
        ...
        "_sampling", "_http_client_config", "_http_client",
    )
    ...
    def __init__(self, name: str):
        ...
        self._http_client_config: Optional[dict] = None
    ...
    async def _run_http(self, config: dict):
        ...
        # Expand environment variables in header values (e.g., ${KAI_JWT_TOKEN})
        if headers:
            headers = {
        ...
        # Use a direct HTTP approach that avoids the SSE GET stream race condition.
        # Some servers (like Kai) return JSON responses directly (enableJsonResponse),
        # making the SSE stream unnecessary for basic tool calls.
        import httpx
        from mcp import types as mcp_types

        # Keep the HTTP client alive for tool calls (session affinity).
        # Force HTTP/1.1 with a single connection to ensure all requests
        # go through the same TCP connection (server ties sessions to connections).
        ...
        try:
            ...
            init_payload = {
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "kai-agent", "version": "0.1.0"},
                },
            }
            ...
            logger.info("MCP server '%s' initialized, session=%s", self.name, session_id)

            if session_id:
                req_headers["mcp-session-id"] = session_id

            # Brief delay to let server register the session
            await asyncio.sleep(1.0)

            # 2. Send initialized notification (may return 400 on some servers, safe to ignore)
            ...
            # Brief delay after notification for server to mark session as initialized
            await asyncio.sleep(1.0)

            # 3. List tools
            tools_payload = {"jsonrpc": "2.0", "id": 2, "method": "tools/list", "params": {}}
            resp = await client.post(url, json=tools_payload, headers=req_headers)
            resp.raise_for_status()
            tools_data = resp.json()

            # Store tools for discovery
            tools_list = tools_data.get("result", {}).get("tools", [])
            self._tools = []
            for t in tools_list:
                self._tools.append(type("MCPTool", (), {
                    "name": t.get("name", ""),
                    "description": t.get("description", ""),
                    "inputSchema": t.get("inputSchema", {}),
                })())

            # Store session info for tool calls
            ...
            logger.info(
                "MCP server '%s' ready: %d tools discovered via HTTP JSON",
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        # Support both session-based and HTTP JSON-based tool calls
        if not server.session and not getattr(server, "_http_client_config", None):
            return json.dumps({
                "error": f"MCP server '{server_name}' is not connected"
            })

        async def _call():
            # HTTP JSON mode (for servers that support enableJsonResponse)
            if getattr(server, "_http_client_config", None) and not server.session:
                import httpx
                cfg = server._http_client_config
                # Create a fresh session per tool call to avoid connection affinity issues.
                # The MCP server ties sessions to TCP connections, so we init + call atomically.
                async with httpx.AsyncClient(timeout=cfg["timeout"]) as tc:
                    base_headers = {k: v for k, v in cfg["headers"].items() if k != "mcp-session-id"}
                    # 1. Initialize
                    init_resp = await tc.post(cfg["url"], json={
                        "jsonrpc": "2.0", "id": 1, "method": "initialize",
                        "params": {"protocolVersion": "2025-03-26", "capabilities": {},
                                   "clientInfo": {"name": "kai-agent", "version": "0.1.0"}},
                    }, headers=base_headers)
                    sid = init_resp.headers.get("mcp-session-id")
                    call_headers = {**base_headers, "mcp-session-id": sid} if sid else base_headers
                    # 2. Brief delay for session registration
                    await asyncio.sleep(1.0)
                    # 3. Notify (ignore errors)
                    try:
                        await tc.post(cfg["url"], json={"jsonrpc": "2.0", "method": "notifications/initialized"}, headers=call_headers)
                    except Exception:
                        pass
                    await asyncio.sleep(0.5)
                    # 4. Call the tool
                    resp = await tc.post(cfg["url"], json={
                ...
                if "error" in data:
                    return json.dumps({"error": _sanitize_error(str(data["error"]))})
                ...
                if is_error:
                    return json.dumps({"error": _sanitize_error(text or "MCP tool returned an error")})
                return json.dumps({"result": text})

            # Standard session-based mode
...
def _make_check_fn(server_name: str):
    ...
    def _check() -> bool:
        ...
        if server is None:
            return False
        # Support both session-based and HTTP JSON-based connections
        return server.session is not None or getattr(server, "_http_client_config", None) is not None
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    ...
    # For the primary Kai server, use shorter prefix (kai_ instead of mcp_kai_)
    if safe_server_name == "kai":
        prefixed_name = f"kai_{safe_tool_name}"
    else:
...
def _build_utility_schemas(server_name: str) -> List[dict]:
    ...
    prefix = f"kai" if safe_name == "kai" else f"mcp_{safe_name}"
    return [
        {
            "schema": {
                "name": f"{prefix}_list_resources",
...
async def _discover_and_register_server(name: str, config: dict) -> List[str]:
    ...
    # Kai server gets a cleaner toolset name
    toolset_name = f"kai" if name == "kai" else f"mcp-{name}"
...
def discover_mcp_tools() -> List[str]:
    ...
    if all_tools:
        # Dynamically inject into all kai-* and hermes-* platform toolsets
