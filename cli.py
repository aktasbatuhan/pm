#!/usr/bin/env python3
"""
Dash PM CLI - Interactive Terminal Interface

A beautiful command-line interface for the Dash PM, autonomous PM agent.
...
from kai_cli import callbacks as _callbacks
...
# ASCII Art - DASH PM logo (full width)
...
# ASCII Art - Dash icon (compact, fits in left panel)
...
COMPACT_BANNER = """
[bold #6A9EBE]╔══════════════════════════════════════════════════════════════╗[/]
[bold #6A9EBE]║[/]  [#EAEAEA]◆ DASH PM[/] [dim #6A9EBE]- Product Management Agent[/]             [bold #6A9EBE]║[/]
[bold #6A9EBE]║[/]  [#BE6A9F]Your AI Product Manager[/]                               [bold #6A9EBE]║[/]
[bold #6A9EBE]╚══════════════════════════════════════════════════════════════╝[/]
"""
...
def _build_compact_banner() -> str:
    ...
    if w < 30:
        return "\n[#6A9EBE]◆ DASH PM[/] [dim #4A7A9E]- Product Management[/]\n"
    inner = w - 2  # inside the box border
    bar = "═" * w
    line1 = "◆ DASH PM - Product Management Agent"
    line2 = "Your AI Product Manager"
    # Truncate and pad to fit
    line1 = line1[:inner - 2].ljust(inner - 2)
    line2 = line2[:inner - 2].ljust(inner - 2)
    return (
        f"\n[bold #6A9EBE]╔{bar}╗[/]\n"
        f"[bold #6A9EBE]║[/] [#EAEAEA]{line1}[/] [bold #6A9EBE]║[/]\n"
        f"[bold #6A9EBE]║[/] [dim #4A7A9E]{line2}[/] [bold #6A9EBE]║[/]\n"
        f"[bold #6A9EBE]╚{bar}╝[/]\n"
    )
...
def build_welcome_banner(console: Console, model: str, cwd: str, tools: List[dict] = None, enabled_toolsets: List[str] = None, session_id: str = None, context_length: int = None):
    ...
    try:
        ...
        _accent = _bskin.get_color("banner_accent", "#6A9EBE")
        _dim = _bskin.get_color("banner_dim", "#4A7A9E")
        _text = _bskin.get_color("banner_text", "#EAEAEA")
        _session_c = _bskin.get_color("session_border", "#8B8682")
        _title_c = _bskin.get_color("banner_title", "#6A9EBE")
        _border_c = _bskin.get_color("banner_border", "#4A7A9E")
        _agent_name = _bskin.get_branding("agent_name", "Dash PM")
    except Exception:
        _bskin = None
        _accent, _dim, _text = "#6A9EBE", "#4A7A9E", "#EAEAEA"
        _session_c, _title_c, _border_c = "#8B8682", "#6A9EBE", "#4A7A9E"
        _agent_name = "Dash PM"
...
class HermesCLI:
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Kai: ", style="dim bold #8FBC8F")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        """
        from kai_cli.clipboard import save_clipboard_image
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: kai --checkpoints")
    ...
    def _handle_paste_command(self):
        ...
        """
        from kai_cli.clipboard import has_clipboard_image
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self.console.print("[dim]   Run 'kai setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from kai_cli.commands import COMMANDS_BY_CATEGORY
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Kai!{_RST}")
    ...
    def _handle_jobs_command(self, cmd: str):
        """Handle the /jobs command — show GitHub PRs, reviews, and issues."""
        try:
            from tools.github_jobs import run_jobs_command
        except ImportError as e:
            print(f"\n  /jobs requires httpx: {e}\n")
            return

        args = cmd.split(maxsplit=1)[1] if " " in cmd else ""
        try:
            output = run_jobs_command(args)
            print()
            for line in output.split("\n"):
                print(f"  {line}")
            print()
        except Exception as e:
            print(f"\n  Error fetching GitHub jobs: {e}\n")
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        from kai_cli.skills_hub import handle_skills_slash
            ...
            try:
                ...
                if response:
                    try:
                        ...
                        _resp_color = _skin.get_color("response_border", "#4A7A9E")
                    except Exception:
                        label = "◆ Kai"
                        _resp_color = "#4A7A9E"
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from kai_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  kai --resume {self.session_id}")
            print()
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Dash PM ready. Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#EAEAEA")
        except Exception:
            _welcome_text = "Dash PM ready. Type your message or /help for commands."
...
def main(
    ...
    """
    Dash PM CLI - Interactive AI Assistant
