...
if _user_env.exists():
    try:
        load_dotenv(dotenv_path=_user_env, override=False, encoding="utf-8")
    except UnicodeDecodeError:
        load_dotenv(dotenv_path=_user_env, override=False, encoding="latin-1")
...
from kai_constants import OPENROUTER_BASE_URL, OPENROUTER_MODELS_URL

# Agent internals extracted to agent/ package for modularity
from agent.prompt_builder import (
    DEFAULT_AGENT_IDENTITY, PLATFORM_HINTS,
    MEMORY_GUIDANCE, SESSION_SEARCH_GUIDANCE, SKILLS_GUIDANCE, WORKSPACE_CONTEXT_GUIDANCE,
    CRONJOB_MONITORING_GUIDANCE, PM_GUIDANCE,
)
...
class AIAgent:
    ...
    def __init__(
        ...
        if not skip_memory:
            try:
                from kai_cli.config import load_config as _load_mem_config
        ...
        try:
            from kai_cli.config import load_config as _load_skills_config
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if get_env("KAI_DUMP_REQUEST_STDOUT").strip().lower() in {"1", "true", "yes", "on"}:
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Workspace context tools — inject guidance when workspace tools are available
        ws_tool_names = {"memory", "todo", "workspace_get_blueprint", "workspace_add_learning"}
        if ws_tool_names & self.valid_tool_names:
            tool_guidance.append(WORKSPACE_CONTEXT_GUIDANCE)
        # Cron monitoring — inject when scheduling tools are available
        cron_tool_names = {"schedule_cronjob", "list_cronjobs", "remove_cronjob"}
        if cron_tool_names & self.valid_tool_names:
            tool_guidance.append(CRONJOB_MONITORING_GUIDANCE)
        # PM guidance — inject when PM brief tools are available
        pm_tool_names = {"brief_store", "brief_get_latest", "brief_get_action_items"}
        if pm_tool_names & self.valid_tool_names:
            tool_guidance.append(PM_GUIDANCE)
        if tool_guidance:
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from kai_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(get_env("KAI_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(get_env("KAI_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _consume_stream(self, stream):
        """Consume an OpenAI streaming response, fire token_callback for each
        content delta, and return a synthetic ChatCompletion-compatible object
        so the rest of run_conversation() works unchanged."""
        from types import SimpleNamespace
        ...
        for chunk in stream:
            ...
            # Tool call fragments (arguments arrive incrementally)
            if delta.tool_calls:
                for tc_delta in delta.tool_calls:
                    ...
                    if idx not in tool_calls_acc:
                        tool_calls_acc[idx] = {
                            "id": tc_delta.id or "",
                            "type": "function",
                            "function": {"name": tc_delta.function.name or "" if tc_delta.function else "", "arguments": ""},
                        }
        ...
        # Build a synthetic response object compatible with ChatCompletion
        ...
        response = SimpleNamespace(
            choices=[choice],
            usage=None,
            model=getattr(stream, "model", self.model),
        )
        return response
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        def _call():
            try:
                ...
                elif self.token_callback and self.api_mode == "chat_completions":
                    # Stream tokens for real-time SSE delivery
                    streaming_kwargs = dict(api_kwargs)
                    streaming_kwargs["stream"] = True
                    stream = self.client.chat.completions.create(**streaming_kwargs)
                    result["response"] = self._consume_stream(stream)
                else:
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if function_name == "todo":
                ...
                # Push learnings to backend for the Memory page
                if self.state_reporter and function_args.get("action") == "add":
                    category = "preference" if target == "user" else "security"
                    self.state_reporter.push_learning(category, function_args.get("content", ""))
                tool_duration = time.time() - tool_start_time
                if self.quiet_mode:
            ...
            # Report tool activity and detect state transitions for the frontend
            if self.state_reporter:
                try:
                    self.state_reporter.report_activity(
                        tool_name=function_name,
                        args=function_args if isinstance(function_args, dict) else {},
                        result_preview=result_preview,
                        duration_ms=tool_duration * 1000,
                    )
                    self.state_reporter.detect_state_transition(
                        function_name,
                        function_args if isinstance(function_args, dict) else {},
                        function_result,
                    )
                except Exception:
                    pass  # Never let reporting break the agent loop
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if get_env("KAI_DUMP_REQUESTS").strip().lower() in {"1", "true", "yes", "on"}:
