...
# MCP tool discovery is deferred until ensure_mcp_discovered() is called.
# This allows the CLI to load .env files before MCP servers need auth tokens.
_mcp_discovered = False

def ensure_mcp_discovered():
    """Discover MCP tools if not already done. Call after env vars are loaded.

    This blocks until discovery is complete (typically 3-5 seconds for HTTP servers).
    """
    ...
    try:
        ...
        if result:
            # Rebuild maps after MCP tools are registered
            ...
            logger.info("MCP discovery complete: %d tools", len(result))
        else:
            logger.info("MCP discovery returned no tools")
    except Exception as e:
...
def get_tool_definitions(
    ...
    """
    # Ensure MCP tools are discovered (deferred until env vars are loaded)
