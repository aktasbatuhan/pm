...
def get_hermes_home() -> Path:
    """Get the Kai Agent home directory (~/.kai-agent).

    Falls back to ~/.hermes for backward compatibility if it exists
    and ~/.kai-agent does not.
    """
    env_home = os.getenv("HERMES_HOME") or os.getenv("KAI_HOME")
    if env_home:
        return Path(env_home)
    kai_home = Path.home() / ".kai-agent"
    hermes_home = Path.home() / ".hermes"
    # Prefer ~/.kai-agent, fall back to ~/.hermes if it exists
    if kai_home.exists():
        return kai_home
    if hermes_home.exists():
        return hermes_home
    return kai_home  # Default for new installs
...
def show_config():
    ...
    print(color("│              ◆ Dash PM Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
