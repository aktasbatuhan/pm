...
Usage:
    ...
    kai chat                # Interactive chat
    kai gateway             # Run gateway in foreground
    kai gateway start       # Start gateway as service
    kai gateway stop        # Stop gateway service
    kai gateway status      # Show gateway status
    kai gateway install     # Install gateway service
    kai gateway uninstall   # Uninstall gateway service
    kai setup               # Interactive setup wizard
    kai logout              # Clear stored authentication
    kai status              # Show status of all components
    kai cron                # Manage cron jobs
    kai cron list           # List cron jobs
    kai cron status         # Check if cron scheduler is running
    kai doctor              # Check configuration and dependencies
    kai version             # Show version
    kai update              # Update to latest version
    kai uninstall           # Uninstall Dash PM
    kai sessions browse     # Interactive session picker with search
    kai claw migrate        # Migrate from OpenClaw to Hermes
    kai claw migrate --dry-run  # Preview migration without changes
"""
...
from kai_cli.config import get_env_path, get_hermes_home
...
def _has_any_provider_configured() -> bool:
    ...
    from kai_cli.config import get_env_path, get_hermes_home
    from kai_cli.auth import get_auth_status

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from kai_cli.auth import PROVIDER_REGISTRY
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'kai sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'kai setup' at any time to configure.")
        sys.exit(1)
...
def cmd_gateway(args):
    ...
    from kai_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from kai_cli.config import get_env_value, save_env_value
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: kai gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  kai gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '◆ Dash'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  kai gateway")
        ...
        print()
        print("  Or install as a service: kai gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'kai whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from kai_cli.setup import run_setup_wizard
    run_setup_wizard(args)


def cmd_model(args):
    """Select default model — starts with provider selection, then model picker."""
    from kai_cli.auth import (
    ...
    from kai_cli.config import load_config, save_config, get_env_value, save_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from kai_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from kai_cli.config import get_env_value, save_env_value
    ...
    from kai_cli.models import model_ids
    ...
    if selected:
        ...
        from kai_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    try:
        from kai_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from kai_cli.auth import _save_model_choice, deactivate_provider
    from kai_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        print("Endpoint saved. Use `/model` in chat or `kai model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from kai_cli.auth import _save_model_choice, deactivate_provider
    from kai_cli.config import save_env_value, load_config, save_config
    from kai_cli.models import fetch_api_models
...
def cmd_login(args):
    ...
    from kai_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from kai_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from kai_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from kai_cli.cron import cron_command
    cron_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from kai_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from kai_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Dash PM v{__version__} ({__release_date__})")
...
def cmd_uninstall(args):
    """Uninstall Dash PM."""
    from kai_cli.uninstall import run_uninstall
    run_uninstall(args)


def _update_via_zip(args):
    """Update Dash PM by downloading a ZIP archive.
...
def cmd_update(args):
    """Update Dash PM to the latest version."""
    import subprocess
    import shutil
    
    print("◆ Updating Dash PM...")
    print()
    ...
    try:
        ...
        if needs_migration:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'kai config migrate' later to configure.")
        else:
        ...
        try:
            ...
            if check.stdout.strip() == "active":
                ...
                if restart.returncode == 0:
                    ...
                    print("  Try manually: kai gateway restart")
        except (FileNotFoundError, subprocess.TimeoutExpired):
            pass  # No systemd (macOS, WSL1, etc.) — skip silently
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  kai model              # Select provider and model")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="kai",
        description="Dash PM - AI-powered product management agent",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    hermes                        Start interactive chat
    kai chat -q "Hello"        Single query mode
    hermes -c                     Resume the most recent session
    hermes -c "my project"        Resume a session by name (latest in lineage)
    kai --resume <session_id>  Resume a specific session by ID
    kai setup                  Run setup wizard
    kai logout                 Clear stored authentication
    kai model                  Select default model
    kai config                 View configuration
    kai config edit            Edit config in $EDITOR
    kai config set model gpt-4 Set a config value
    kai gateway                Run messaging gateway
    hermes -w                     Start in isolated git worktree
    kai gateway install        Install as system service
    kai sessions list          List past sessions
    kai sessions browse        Interactive session picker
    kai sessions rename ID T   Rename/title a session
    kai update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Dash PM"
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Dash PM with an interactive wizard. "
                    "Run a specific section: kai setup model|terminal|gateway|tools|agent"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Dash PM components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Dash PM setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Dash PM configuration"
    )
    ...
    def cmd_pairing(args):
        from kai_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            from kai_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from kai_cli.skills_hub import skills_command
            skills_command(args)
    ...
    def cmd_tools(args):
        from kai_cli.tools_config import tools_command
        tools_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch kai --resume <id> by replacing the current process
    ...
    def cmd_claw(args):
        from kai_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Dash PM to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Dash PM",
        description="Remove Dash PM from your system. Can keep configs/data for reinstall."
    )
