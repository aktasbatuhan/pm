...
from kai_cli.colors import Colors, color
...
def setup_model_provider(config: dict):
    ...
    if provider_idx == 0:  # Nous Portal (OAuth)
        ...
        try:
            from kai_cli.auth import _login_nous, ProviderConfig
            import argparse
        ...
        try:
            from kai_cli.auth import deactivate_provider
        ...
        if api_key:
            ...
            from kai_cli.auth import detect_zai_endpoint
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "nous" and nous_models:
            ...
            from kai_cli.models import model_ids, menu_labels
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ◆ Dash PM Setup — {label:<37s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    from kai_cli.auth import get_active_provider
    ...
    print(
        color(
            "│             ◆ Dash PM Setup Wizard                      │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Dash PM installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
