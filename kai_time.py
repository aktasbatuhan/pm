"""
Timezone-aware clock for Kai Agent.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``KAI_TIMEZONE`` environment variable (falls back to ``HERMES_TIMEZONE``)
  2. ``timezone`` key in config.yaml
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely.
"""
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = get_env("KAI_TIMEZONE").strip()
    if tz_env:
        return tz_env
